...
public function googleSheet()
{
    $columns = [
        ...
        'governorate'       => (int) Setting::getValue('column_index_governorate', 3), // جديد
        'tracking_number'   => (int) Setting::getValue('column_index_tracking_number', 10), // جديد
        'credentials_uploaded' => \Storage::exists('google/credentials.json'),
    ];

    // الغلط هنا: ما فيش متغير $settings متعرف
    // الصح:
...
public function updateGoogleSheet(Request $request)
{
    ...
    $keys = [
        ...
        'product_name_column',
        'color_type_column',
        'governorate_column',          // ⬅️ هنا
        'tracking_number_column',      // ⬅️ وهنا
    ];
...
foreach ([
    ...
    'governorate'       => $request->input('governorate_column'),        // ⬅️ أضف هذا
    'tracking_number'   => $request->input('tracking_number_column'),    // ⬅️ وأضف هذا
] as $key => $letter) {
